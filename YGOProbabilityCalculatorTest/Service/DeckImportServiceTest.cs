...
public class DeckImportServiceTest {
    ...
    [Test]
    public async Task ImportDeckFromYdkAsync_ValidFile_ReturnsCorrectDeckEntries() {
        ...
        try {
            ...
            var firstCard = result.First(x => x.Name == "Test Card 1");
            Assert.Multiple(() => {
                Assert.That(firstCard.Copies, Is.EqualTo(2));
                Assert.That(firstCard.Categories, Is.Empty);
            });

            var secondCard = result.First(x => x.Name == "Test Card 2");
            Assert.Multiple(() => {
    ...
    [Test]
    public async Task ImportDeckFromYdkAsync_CardInfoServiceFails_UsesIdAsName() {
        var tempFile = Path.GetTempFileName();
        await File.WriteAllLinesAsync(tempFile, [
            "#main",
            "12345"
        ...
        try {
            ...
            var card = result[0];
            Assert.Multiple(() => {
                Assert.That(card.Name, Is.EqualTo("12345"));
    ...
    public async Task ImportDeckFromYdkAsync_InvalidCardId_SkipsInvalidLines() {
        ...
        try {
            ...
            var card = result[0];
            Assert.Multiple(() => {
                Assert.That(card.Name, Is.EqualTo("Test Card"));
                Assert.That(card.Copies, Is.EqualTo(1));
                Assert.That(card.Categories, Is.Empty);
            });
        }
